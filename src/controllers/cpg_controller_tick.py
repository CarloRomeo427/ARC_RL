...
_KNEE_STANCE_DEG  = 0.0
_ANKLE_STANCE_DEG = 0.0


def _stance_pose(leg: str) -> Dict[str, float]:
    return {'hip': 0.0, 'knee': _KNEE_STANCE_DEG * _D, 'ankle': _ANKLE_STANCE_DEG * _D}
...
def reset_env_to_stance(env, seed: int = None) -> np.ndarray:
    ...
    qpos[2] += u.np_random.uniform(-0.015, 0.015)
...
class ScriptedCPGControllerTick:

    PRESETS = {
        'slow':   dict(freq=1.5, duty=0.65, hip_amp_deg=32,
                       knee_swing_flex_deg=-35,
                       knee_stance_extend_deg=+8,
                       ankle_lift_deg=35),
        'medium': dict(freq=1.5, duty=0.60, hip_amp_deg=40,
                       knee_swing_flex_deg=-40,
                       knee_stance_extend_deg=+12,
                       ankle_lift_deg=45),
        'fast':   dict(freq=1.5, duty=0.55, hip_amp_deg=40,
                       knee_swing_flex_deg=-45,
                       knee_stance_extend_deg=+18,
                       ankle_lift_deg=55),
    }

    _PHASE_INIT = 0.0

    def __init__(self, env, robot: str = 'tick',
                 preset: str = 'medium',
                 ...
                 settle_steps: int = 15):
        if robot.lower() != 'tick':
            raise ValueError(f"TickController tuned for Tick only. Got robot='{robot}'.")
        ...
        self.ankle_lift         = p['ankle_lift_deg']         * _D
    ...
    def _leg_targets(self, leg: str, phi: float):
        ...
        if u < self.duty:
            ...
            ankle_tgt = stance_pose['ankle'] + self.ankle_lift * swing_bell
...
def collect_episode(env, ctrl: ScriptedCPGControllerTick,
...
def save_demos_hdf5(env_name: str, out_path: str, n_episodes: int = 20,
    ...
    ctrl = ScriptedCPGControllerTick(env, robot=env_name, preset=preset)
    returns = []
    ...
    env.close()
    print(f"[cpg-tick] {env_name} ({preset}): {n_episodes} eps -> {out_path} | "
...
def record_video(env_name: str, out_path: str, max_ep_len: int = 1000,
    ...
    print(f"[cpg-tick] {env_name} ({preset}): {len(frames)} frames, "
...
def seed_replay_buffer(buffer, env_name: str, n_transitions: int,
    ...
    while pushed < n_transitions:
        ...
        ep += 1
    env.close()
    print(f"[cpg-tick] seeded buffer with {pushed} scripted transitions "
          f"from {env_name} ({preset})")
    return pushed


def imitation_reward(ctrl: ScriptedCPGControllerTick,
...
def _parse():
    p = argparse.ArgumentParser(
        description="Alternating-tripod CPG for the ARC-RL Tick"
    )
    p.add_argument('--env', type=str, default='tick', choices=['tick'])
...
def main():
    ...
    if not args.video and not args.out:
        ...
        ctrl = ScriptedCPGControllerTick(env, robot=args.env, preset=args.preset)
        ep = collect_episode(env, ctrl, args.max_ep_len, seed=args.seed)
        env.close()
        print(f"[cpg-tick] {args.env} ({args.preset}): "
