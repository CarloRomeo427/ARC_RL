...
_XML_PATH = os.path.join(os.path.dirname(__file__), "..", "..", "src/models", "bastion_v1.xml")

DEFAULT_CAMERA_CONFIG = {
    "distance": 4.0,
}

class BastionEnv(MujocoEnv, utils.EzPickle):
    ...
    def __init__(
        ...
        healthy_z_range: Tuple[float, float] = (0.12, 1.0),
        ...
        **kwargs,
        ...
        utils.EzPickle.__init__(
            ...
            orientation_weight, gait_frequency, main_body, terminate_when_unhealthy, 
    ...
    def _gait_error(self):
        """Returns the raw number of legs violating the gait clock (0 to 6)."""
        error_count = 0.0
        for i, bid in enumerate(self._foot_body_ids):
            ...
            # CLAUDE'S FIX: 0.12m threshold so the Bastion's feet actually register as grounded!
    ...
    def _get_rew(self, x_velocity: float, action):
        # 1. Calculate Gait Compliance (1.0 = Perfect, 0.0 = All legs wrong)
        gait_errors = self._gait_error()
        compliance = 1.0 - (gait_errors / len(self.FOOT_NAMES))
        
        # 2. Gate the Velocity Reward: The fix that forced correct middle leg use!
        forward_reward = (x_velocity * self._forward_reward_weight) * compliance
        healthy_reward = self.healthy_reward

        # 3. Tripod Minimum Check (CLAUDE'S FIX: Uses 0.12 threshold)
        legs_on_ground = sum(1 for bid in self._foot_body_ids if self.data.body(bid).xpos[2] < 0.12)
        tripod_penalty = 0.0
        if legs_on_ground < 3:
            tripod_penalty = 2.0 * (3 - legs_on_ground) 
